...
class MarkerManager {
     ...
     */
    fun update(userMarker: UserMarker) {
        current[userMarker.id]!!.position = userMarker.position
    }
    ...
    internal class Colors {
        ...
        init {
            available.addAll(arrayOf(brown_marker, magenta_marker, orange_marker, green_marker, red_marker, blue_marker))
        }

        fun pick(id: String): Int = available.pop().also { assigned[id] = it }

        fun release(id: String) {
            available.push(assigned.remove(id))
        }
